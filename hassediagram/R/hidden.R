...
sortaov.term.fun <- function(x, no.all) {
...
confound.tab.fun <- function(no.confounded, no.all, all.nestedin.conf.table, conf.nestedin.all.table,
...
designorder <- function (level, orderdsi, designi_1, order.effectsi_1, level.order.effectsi_1, 
                         keep.nested.effectsi_1, keep.nested.interactionsi_1, keep.order.effectsiuniqi_1, 
  ...
  if (length(keep.nested.effectsi_1) >1 || keep.nested.effectsi_1 != "") {
    if (length(order.effectsi) > 1 || order.effectsi !="") {
      order.effectsi <- c(order.effectsi,keep.nested.effectsi_1)
    } else {
  ...
  if (length(keep.order.effectsiuniqi_1) >1 || keep.order.effectsiuniqi_1 != "") {
    if (length(order.effectsi) > 1 || order.effectsi !="") {
      order.effectsi <- c(order.effectsi,keep.order.effectsiuniqi_1)
    } else {
...
dscoords.fun <- function(DStype, finaleffects, ceffects.table.final.brief, larger.fontlabelmultiplier,
...
dfs.fun <- function(DStype, no.all, finaleffects, ceffects.table.final.brief, adjm, outputlistip1, maxfacs,
                    maxlevels.df, check.shared.df, datadesign) {
  if (DStype=="LS") {
    xfinaleffects <- finaleffects
    xfinaldesign.effects <- names(finaleffects)
    xadjm <- adjm
      ...
      if (xDFsDiff != 0) {
        ...
        for (i in 2:length(tiers)) {
          ...
          if (any(grepl(":",rownames(droptermi)[-1]))) {        
            ...
            #zzz^change
            namedropi <- gsub("\\:","\\^",sortaov.term.fun(as.vector(strip.order.fun(rownames(droptermi)[-1])), no.all))
            #zzz^change
          } else {
        ...
        for (i in 1:nrow(tier.dfs)) {
          if (tier.dfs[i,1] > sharetier) {
            ...
            if (any(grepl(":",names(anovai.df)))) {        
              #checks of any terms are interactions
              #zzz^change
              nameanovai <- gsub("\\:","\\^",sortaov.term.fun(as.vector(strip.order.fun(names(anovai.df))), no.all))
              #zzz^change
            } else {
